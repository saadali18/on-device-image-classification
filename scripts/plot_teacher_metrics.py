...
def plot_loss(metrics_df: pd.DataFrame, output_dir: str):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title(f"Teacher {MODEL_NAME} CIFAR-100 Loss Curve")
    plt.legend()
...
def plot_accuracy(metrics_df: pd.DataFrame, output_dir: str):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy (%)")
    plt.title(f"Teacher {MODEL_NAME} CIFAR-100 Accuracy Curve")
    plt.legend()
    plt.grid(True, alpha=0.3)
...
def plot_top1_accuracy(metrics_df: pd.DataFrame, output_dir: str):
    ...
    plt.title(f"Teacher {MODEL_NAME} CIFAR-100 Top-1 Accuracy")
    plt.legend()
    plt.grid(True, alpha=0.3)
...
def plot_learning_rate(metrics_df: pd.DataFrame, output_dir: str):
    ...
    plt.title(f"Teacher {MODEL_NAME} Learning Rate Schedule")
    plt.legend()
    plt.grid(True, alpha=0.3)
...
def main():
    ...
    if not os.path.exists(metrics_path):
        raise FileNotFoundError(
            ...
            "Run teacher training first: python scripts/train_teacher.py"
        )
